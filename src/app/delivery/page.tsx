...
type Order = { order_id:string; platform:string|null; first_order_date:string|null; nickname:string|null; order_status:string|null; order_count:number|null; created_at:string|null; domestic_shipping:Shipping|Shipping[]|null };
...
export default function DeliveryPage(){
 ...
 return <main className="delivery-shell">
  ...
  <section className="delivery-content compact-content">
   ...
   <div className="delivery-list compact-list">{filtered.map(order=>{const s=shipping(order);const firstDate=order.first_order_date||order.created_at;const days=daysSince(firstDate);const isKeep=order.order_status==="kept";const currentStatus=statusText(s?.shipping_status);const count=Math.max(1,Number(order.order_count)||1);return <details className="delivery-row-card" key={order.order_id}>
    <summary className="delivery-row-summary">
     ...
     <span className="summary-status"><i className="status-dot"/>{currentStatus}</span>
     <span className="order-count"><small>합배송</small><b>{count}건</b></span>
     <span className="summary-chevron">▼</span>
    </summary>
    <div className="delivery-row-detail"><dl className="delivery-info compact-info"><div><dt>최초주문일</dt><dd>{dateText(firstDate)}</dd></div><div><dt>배송상태</dt><dd><span className="status-dot"/>{currentStatus}</dd></div><div><dt>운송장</dt><dd className="tracking">{s?.tracking_number||"아직 등록 전"}</dd></div><div><dt>KEEP 여부</dt><dd>{isKeep?"KEEP 중":"아니오"}</dd></div></dl></div>
   </details>})}</div>
  </section>
